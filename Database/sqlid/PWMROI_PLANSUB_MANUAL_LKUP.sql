...
set define off
prompt ---------------------------------------;
prompt Executing ... PWMROI_PLANSUB_MANUAL_LKUP.sql
prompt ---------------------------------------;
declare
v_folder_id sfcore_sql_lib_folder.folder_id%type  :='PWUST_550FF0D87F4141A0E05387971F0AD3EA';
v_sql_id sfcore_sql_lib.sql_id%type :='PWMROI_PLANSUB_MANUAL_LKUP';
v_sql_id_displ sfcore_sql_lib.sql_id_displ%type  :='PWMROI_PLANSUB_MANUAL_LKUP';
...
where not exists (select manual_no from PWMROI_PLAN_SUBJECT_AUTHORITY B WHERE B.PLAN_ID = :PLAN_ID AND B.PLAN_REVISION = :PLAN_REVISION AND
