...
set define off
prompt ---------------------------------------;
prompt Executing ... PWUST_RPTTRAVELERORDERTASKGROUPS.sql
prompt ---------------------------------------;
declare
v_folder_id sfcore_sql_lib_folder.folder_id%type  :='PWUST_550FF0D87F6141A0E05387971F0AD3EA';
v_sql_id sfcore_sql_lib.sql_id%type :='PWUST_RPTTRAVELERORDERTASKGROUPS';
v_sql_id_displ sfcore_sql_lib.sql_id_displ%type  :='PWUST_RptTravelerOrderTaskGroups';
v_read_only sfcore_sql_lib.read_only%type  :='';
v_datasource sfcore_sql_lib.datasource%type  :='';
v_stype sfcore_sql_lib.stype%type  :='';
v_description sfcore_sql_lib.description%type  :='SMRO_WOE_310; 1566; 1371';
v_sql_text sfcore_sql_lib.sql_text%type  :='select order_id, subject_no, subject_rev, title as subject_title,  '''' as security_group
from SFOR_SFWID_ORDER_SUBJECT
where ORDER_ID = :Order_id
and included_flag = ''INCLUDED''
union
select OD.order_id, 1 as subject_no, 1 as subject_rev, ''SUPPLEMENTAL TASKS'' as subject_title, od.security_group
from  SFMFG.SFWID_ORDER_DESC OD
where od.order_type = ''Supplemental''
