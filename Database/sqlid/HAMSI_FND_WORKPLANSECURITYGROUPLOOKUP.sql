...
v_sql_id varchar2(85) :='HAMSI_FND_WORKPLANSECURITYGROUPLOOKUP';
v_sql_id_displ varchar2(85) :='HAMSI_FND_WorkPlanSecurityGroupLookup';
v_read_only number :='';
v_datasource varchar2(32) :='';
v_stype varchar2(32) :='SFPL';
v_site varchar2(10) :='HAMSI';
v_description varchar2(200) :='Cloned from MFI_FND_WorkPlanSecurityGroupLookup';
...
WHERE
    ...
    AND SECURITY_GROUP NOT IN (SELECT COLUMN_VALUE
                               FROM  TABLE(SFFND_PARSE_FCN((SELECT SECURITY_GROUP FROM SFPL_PLAN_DESC WHERE PLAN_NO = :PLAN_NO AND PLAN_UPDT_NO = :PLAN_UPDT_NO), '','')))
