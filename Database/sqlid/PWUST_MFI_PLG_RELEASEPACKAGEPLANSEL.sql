...
set define off
prompt ---------------------------------------;
prompt Executing ... PWUST_MFI_PLG_RELEASEPACKAGEPLANSEL.sql
prompt ---------------------------------------;
declare
v_folder_id sfcore_sql_lib_folder.folder_id%type  :='PWUST_550FF0D87F6141A0E05387971F0AD3EA';
v_sql_id sfcore_sql_lib.sql_id%type :='PWUST_MFI_PLG_RELEASEPACKAGEPLANSEL';
v_sql_id_displ sfcore_sql_lib.sql_id_displ%type  :='PWUST_MFI_PLG_ReleasePackagePlanSel';
v_read_only sfcore_sql_lib.read_only%type  :='';
v_datasource sfcore_sql_lib.datasource%type  :='';
v_stype sfcore_sql_lib.stype%type  :='';
v_description sfcore_sql_lib.description%type  :='Release Package Process Plan Select,SMRO_PLG_301';
v_sql_text sfcore_sql_lib.sql_text%type  :='SELECT TO_PWP_ID,
      PWP_ID,
      PLAN_NO,
      PLAN_ID,
      PLAN_TYPE,
...
FROM (  SELECT  '''' AS TO_PWP_ID,
                ...
                F.CHANGE_TYPE,
                A.UCF_PLAN_VCH3
        FROM    SFPL_PLAN_DESC A, SFPL_PLAN_REV B  LEFT OUTER JOIN SFPL_PLANNED_ACTIONS C ON
  ...
  WHERE  A.PLAN_ID = B.PLAN_ID AND
         ...
         D.PLAN_ALTERATIONS = B.PLAN_ALTERATIONS ) X
WHERE 1 = 1 /*+ @Filter() */';
begin
begin
