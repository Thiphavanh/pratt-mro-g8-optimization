...
set define off
prompt ---------------------------------------;
prompt Executing ... PWMROI_SFOR_SFPLPLANSUBJECT_SEL.sql
prompt ---------------------------------------;
declare
v_folder_id sfcore_sql_lib_folder.folder_id%type  :='PWMROI_721CCDAC8C2341B8A268719CCA67FBCE';
v_sql_id sfcore_sql_lib.sql_id%type :='PWMROI_SFOR_SFPLPLANSUBJECT_SEL';
v_sql_id_displ sfcore_sql_lib.sql_id_displ%type  :='PWMROI_SFOR_SfplPlanSubject_Sel';
v_read_only sfcore_sql_lib.read_only%type  :='';
v_datasource sfcore_sql_lib.datasource%type  :='';
v_stype sfcore_sql_lib.stype%type  :='';
v_description sfcore_sql_lib.description%type  :='SFOR';
v_sql_text sfcore_sql_lib.sql_text%type  :='SELECT A.OBJECT_ID, A.SUBJECT_NO, A.SUBJECT_REV, A.SUBJECT_REV AS MAX_REVISION, A.SUBJECT_STATE, A.TITLE,A.AUTHORITY,
A.UPDT_USERID, A.TIME_STAMP, A.IS_DEFAULT, A.LAST_ACTION,
