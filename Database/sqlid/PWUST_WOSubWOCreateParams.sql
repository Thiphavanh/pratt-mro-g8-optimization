...
set define off
prompt ---------------------------------------;
prompt Executing ... PWUST_WOSUBWOCREATEPARAMS.sql
prompt ---------------------------------------;
declare
v_folder_id sfcore_sql_lib_folder.folder_id%type  :='SFCORE_SQLLIBS';
v_sql_id sfcore_sql_lib.sql_id%type :='PWUST_WOSUBWOCREATEPARAMS';
v_sql_id_displ sfcore_sql_lib.sql_id_displ%type  :='PWUST_WOSubWOCreateParams';
...
v_sql_text sfcore_sql_lib.sql_text%type  :='select ''LOT_NO=''           || COALESCE(sffnd_value_wrapper(LOT_NO), '''') ||
       ...
       '',ORDER_TYPE=''      ||sffnd_value_wrapper(case
                             when exists (select doc_sub_type
                                    ...
                                    where a.doc_sub_type = ''Sub Order''
