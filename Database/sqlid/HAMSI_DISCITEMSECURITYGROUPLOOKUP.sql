...
v_sql_id varchar2(85) :='HAMSI_DISCITEMSECURITYGROUPLOOKUP';
v_sql_id_displ varchar2(85) :='HAMSI_DiscItemSecurityGroupLookup';
v_read_only number :='';
v_datasource varchar2(32) :='';
v_stype varchar2(32) :='SFQA';
v_site varchar2(10) :='HAMSI';
v_description varchar2(200) :='Clonned From MFI_DISCITEMSECURITYGROUPLOOKUP';
...
WHERE
    ...
    AND SECURITY_GROUP NOT IN (SELECT SECURITY_GROUP
                            FROM  SFQA_DISC_ITEM_SEC_GRP WHERE DISC_ID = :DISC_ID AND DISC_LINE_NO = :DISC_LINE_NO)
