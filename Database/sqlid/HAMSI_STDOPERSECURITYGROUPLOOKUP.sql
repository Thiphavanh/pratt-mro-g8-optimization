...
v_sql_id varchar2(85) :='HAMSI_STDOPERSECURITYGROUPLOOKUP';
v_sql_id_displ varchar2(85) :='HAMSI_StdOperSecurityGroupLookup';
v_read_only number :='';
v_datasource varchar2(32) :='';
v_stype varchar2(32) :='SFPL';
v_site varchar2(10) :='HAMSI';
v_description varchar2(200) :='Clonned From MFI_StdOperSecurityGroupLookup';
...
WHERE
    OBSOLETE_RECORD_FLAG != ''Y'' AND
    SECURITY_GROUP NOT IN (
        SELECT SECURITY_GROUP FROM SFCORE_MM_OBJECT_SEC_GRP WHERE OBJECT_ID = :STDOPER_OBJECT_ID) AND
    SECURITY_GROUP IN (
        SELECT SECURITY_GROUP FROM SFFND_USER_SEC_GRP WHERE USERID = :@USERID AND EFFECTIVE_START_DATE < SYSDATE AND EFFECTIVE_END_DATE > SYSDATE )
