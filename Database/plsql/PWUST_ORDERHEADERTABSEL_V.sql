...
-- Created by X006640 on 6/23/2020, 12:36:04 PM --
--------------------------------------------------

set define off

prompt
prompt Creating view PWUST_ORDERHEADERTABSEL_V
prompt =======================================
prompt
CREATE OR REPLACE FORCE VIEW SFMFG.PWUST_ORDERHEADERTABSEL_V AS
SELECT ALT.ALT_NO,
       TT.ORDER_ID,
       ...
       TT.SUBNET,
       -- defect 726 start
      (SELECT CC.CUSTOMER FROM PWUST_SALES_ORDER CC WHERE CC.SALES_ORDER = TT.SALES_ORDER AND ROWNUM = 1) AS CUSTOMER,
      (SELECT CC1.CUSTOMER_DESC FROM PWUST_SALES_ORDER CC1 WHERE CC1.SALES_ORDER = TT.SALES_ORDER AND ROWNUM = 1) CUSTOMER_DESC,
...
grant select on SFMFG.PWUST_ORDERHEADERTABSEL_V to PRATT_READ_ONLY;
grant select, insert, update, delete on SFMFG.PWUST_ORDERHEADERTABSEL_V to SF$CONNECTION_POOL;
